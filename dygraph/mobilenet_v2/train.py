...
def train_mobilenet():
    ...
    with fluid.dygraph.guard(place):
        ...
        train_reader = paddle.batch(paddle.dataset.flowers.train(use_xmap=False), batch_size=args.batch_size, drop_last=True)
        test_reader = paddle.batch(paddle.dataset.flowers.test(use_xmap=False), batch_size=args.batch_size, drop_last=True)
        ...
        for eop in range(1):
            ...
            for img, label in train_data_loader():
                ...
                img = to_variable(np.array([x.reshape(3, 224, 224) for x in img]))
                t1 = time.time()
                label = to_variable(label.numpy().astype('int64').reshape(
                    int(args.batch_size //
