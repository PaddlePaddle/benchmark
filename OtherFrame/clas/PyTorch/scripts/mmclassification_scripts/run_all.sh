...
for i in $(seq 0 `expr ${#model_names[*]} - 1`);do
    ...
    sleep 20
    CUDA_VISIBLE_DEVICES=0 bash run_benchmark.sh ${run_mode} ${_batch_size_full} ${presion} ${_model_name} ${_config_path}
    sleep 20
    ...
    sleep 20
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_benchmark.sh ${run_mode} ${_batch_size_full} ${presion} ${_model_name} ${_config_path}
    sleep 20
done
