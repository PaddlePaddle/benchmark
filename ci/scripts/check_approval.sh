...
do
  if [ -n "Extra open brace or missing close bracefile]}" ]
  then
    need_approval=1
    approval_user=""
    for user in Extra open brace or missing close bracefile]}
    do
      [ -n "Extra open brace or missing close braceuser]}" ] && need_approval=0 && break
