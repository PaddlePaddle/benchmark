...
dy_mobilenet(){
    ...
    do
        ...
        CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_benchmark.sh 1 sp 1000 ${model_item} | tee ${log_path}/dynamic_${model_item}_speed_8gpus 2>&1
    done
}
...
dy_resnet(){
    ...
    CUDA_VISIBLE_DEVICES=5 bash run_benchmark.sh 1 sp 400 | tee ${log_path}/dynamic_${FUNCNAME}_speed_1gpus 2>&1
    sleep 60
    echo "index is speed, begin, muti gpu"
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_benchmark.sh 1 sp 20 | tee ${log_path}/dynamic_${FUNCNAME}_speed_8gpus 2>&1
}
...
#    sleep 60
#    echo "index is speed, begin, muti gpu"
#    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_benchmark.sh 1 sp 1000 | tee ${log_path}/dynamic_${FUNCNAME}_speed_8gpus 2>&1
}
