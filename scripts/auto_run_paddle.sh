...
#    cur_model_path=${BENCHMARK_ROOT}/models/PaddleNLP/language_representations_kit/BERT
    ...
    for model_mode in ${model_mode_list[@]}; do
        for fp_mode in ${fp_mode_list[@]}; do
            ...
            CUDA_VISIBLE_DEVICES=0 bash run_benchmark.sh speed ${model_mode} ${fp_mode} sp 0 ${train_log_dir} ${profiler_log_dir} | tee ${log_path}/${model_name}_speed_1gpus 2>&1
            sleep 60
            echo "index is speed, 8gpus, begin, ${model_name}"
            CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_benchmark.sh speed ${model_mode} ${fp_mode} sp 0 ${train_log_dir} ${profiler_log_dir} | tee ${log_path}/${model_name}_speed_8gpus 2>&1
            sleep 60
            echo "index is mem, 1gpus, begin, ${model_name}"
            CUDA_VISIBLE_DEVICES=0 bash run_benchmark.sh mem ${model_mode} ${fp_mode} sp 0 ${train_log_dir} ${profiler_log_dir} | tee ${log_path}/${model_name}_mem_1gpus 2>&1
            sleep 60
            echo "index is mem, 8gpus, begin, ${model_name}"
            CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_benchmark.sh mem ${model_mode} ${fp_mode} sp 0 ${train_log_dir} ${profiler_log_dir} | tee ${log_path}/${model_name}_mem_8gpus 2>&1
            sleep 60
            #echo "index is maxbs, 1gpus, begin, ${model_name}"
            ...
            #sleep 60
            echo "index is speed, 8gpus, run_mode is multi_process, begin, ${model_name}"
            CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_benchmark.sh speed ${model_mode} ${fp_mode} mp 0 ${train_log_dir} ${profiler_log_dir} | tee ${log_path}/${model_name}_speed_8gpus8p 2>&1
            sleep 60
            echo "index is speed, 1gpus, begin, profiler is 1 ${model_name}"
            CUDA_VISIBLE_DEVICES=0 bash run_benchmark.sh speed ${model_mode} ${fp_mode} sp 1 ${train_log_dir} ${profiler_log_dir} | tee ${profiler_log_dir}/${model_name}_speed_1gpus
2>&1
            sleep
