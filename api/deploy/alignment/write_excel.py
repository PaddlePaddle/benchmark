...
def dump_excel(benchmark_result_list,
    ...
    if url_prefix1:
        print("url prefix2.0: ", url_prefix1)
    if url_prefix2:
        print("url prefix1.8: ", url_prefix2)
    for device in ["gpu", "cpu"]:
        for direction in ["forward", "backward"]:
            ...
            for key in time_set:
                title_names.append("Paddle2.0(%s)" % key)
                title_names.append("Paddle1.8(%s)" % key)
            ...
            for case_id in range(len(benchmark_result_list)):
                ...
                for key in time_set:
                    ...
                    for framework in ["paddle", "tensorflow"]:
                        ...
                        op_speed_path1 = _op_result_path(
                            op_result_dir1, op_unit.case_name, "paddle",
                            device, "speed", direction)
                        op_speed_path2 = _op_result_path(
                            op_result_dir2, op_unit.case_name, "paddle",
                            device, "speed", direction)
#                        if url_prefix1 and os.path.exists(op_speed_path1):
                        if framework=='paddle':
                            op_speed_url1 = _op_result_url(
                                url_prefix1, op_unit.case_name, 'paddle',
                                device, "speed", direction)
                            ws.write_url(
                                row,
                                col,
...
#                        if url_prefix2 and os.path.exists(op_speed_path2):
                        else: 
                            op_speed_url2 = _op_result_url(
                                url_prefix2, op_unit.case_name, 'paddle',
                                device, "speed", direction)
                            ws.write_url(
                                row,
                                col,
...
#                            ws.write(row, col, op_time, cell_formats[color])
                ...
                op_acc_path = _op_result_path(op_result_dir1, op_unit.case_name,
                                              "paddle", device, "accuracy",
                                              direction)
                difference = result["difference"]
                if difference != "--" and difference != "-" and difference != "0.0":
                    difference = "%.2E" % (float(difference))
                if url_prefix1 and os.path.exists(op_acc_path):
                    op_acc_url = _op_result_url(url_prefix1, op_unit.case_name,
                                                "paddle", device, "accuracy",
                                                direction)
                    ws.write_url(
                        row,
