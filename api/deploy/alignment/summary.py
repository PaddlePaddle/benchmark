...
def _parse_speed(version, case_name, statistic_type, last_line):
    assert res.get(case_name, None) is not None

    gpu_time_key = None
    if statistic_type == "2.0paddle_gpu_speed_forward":
        gpu_time_key = "gpu_time"
    elif statistic_type == "2.0paddle_gpu_speed_backward":
        gpu_time_key = "gpu_time_backward"
    elif statistic_type == "1.8paddle_gpu_speed_forward":
        gpu_time_key = "tf_gpu_time"
    elif statistic_type == "1.8paddle_gpu_speed_backward":
...
def get_job_res(version, inputfile, specified_op_list=None):
    """
    ...
    statistic_type = version + file_name[statistic_beg_idx + 1:]
#    statistic_type = file_name[statistic_beg_idx + 1:]
    ...
    if last_line and "_speed_" in statistic_type:
        _parse_speed(version, case_name, statistic_type, last_line)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Specify the Paddle1.8 result directory of operator benchmark.')
    parser.add_argument(
        '--specified_op_list',
        type=str,
        default=None,
    ...
    op_result_dir1 = os.path.abspath(args.op_result_dir1)
    assert os.path.exists(
        op_result_dir1), "Directory %s does not exist." % op_result_dir1
    
    op_result_dir2 = os.path.abspath(args.op_result_dir2)
    assert os.path.exists(
        op_result_dir2), "Directory %s does not exist." % op_result_dir2

    filenames1 = os.listdir(op_result_dir1)
    filenames2 = os.listdir(op_result_dir2)
    if "api_info.txt" in filenames1:
        filenames.remove('api_info.txt')
    assert len(filenames1) > 0, "Directory %s is empty." % op_result_dir1
    assert len(filenames2) > 0, "Directory %s is empty." % op_result_dir2
    specified_op_list = None
    if args.specified_op_list:
        specified_op_list = args.specified_op_list.split()

    for filename in sorted(filenames1):
        get_job_res('2.0', os.path.join(op_result_dir1, filename), specified_op_list)
    for filename in sorted(filenames2):
        print(filename)
        get_job_res('1.8', os.path.join(op_result_dir2, filename), specified_op_list)
    data = []
    ...
    if args.dump_to_excel:
        import write_excel
        print("benchmark_result_list:{}".format(benchmark_result_list))
        write_excel.dump_excel(benchmark_result_list, op_result_dir1,
                               args.url_prefix1, op_result_dir2,
                               args.url_prefix2, args.output_path,
                               op_frequency_dict)

#        write_excel.dump_excel(benchmark_result_list, op_result_dir2,
#                               args.url_prefix2, args.output_path,
#                               op_frequency_dict)
