...
class OpBenchmarkUnit(object):
        ...
        for device in ["gpu", "cpu"]:
            for direction in ["forward", "backward"]:
                ...
                paddle_total, paddle_gpu_time = self._get_case_value(
                    case_detail, "2.0paddle", device, "speed", direction)
                result["paddle"] = {
                    "total": paddle_total,
                ...
                tf_total, tf_gpu_time = self._get_case_value(
                    case_detail, "1.8paddle", device, "speed", direction)
                result["tensorflow"] = {
                    "total": tf_total,
                ...
                accuracy, difference = self._get_case_value(
                    case_detail, "2.0paddle", device, "accuracy", direction)
    ...
    def _get_case_value(self, case_detail, framework, device, task, direction):
        assert framework in ["1.8paddle", "2.0paddle"]
        ...
        if task == "accuracy":
            ...
            try:
                ...
                framework_alias = "" if framework == "2.0paddle" else "tf_"
