...
@benchmark_registry.register("remainder")
...
@benchmark_registry.register("remainder")
class PaddleRemainder(PaddleOpBenchmarkBase):
...
@benchmark_registry.register("remainder")
class TorchRemainder(PytorchOpBenchmarkBase):
    def build_graph(self, config):
        x = self.variable(name='x', shape=config.x_shape, dtype=config.x_dtype)
        y = self.variable(name='y', shape=config.y_shape, dtype=config.y_dtype)
        y = y.detach()
...
@benchmark_registry.register("remainder")
class TFRemainder(TensorflowOpBenchmarkBase):
    def build_graph(self, config):
        x = self.variable(name='x', shape=config.x_shape, dtype=config.x_dtype)
        y = self.variable(name='y', shape=config.y_shape, dtype=config.y_dtype)
