...
class PaddleDynamicSchedulingBenchmarkBase(object):
    def __init__(self):
    ...
    def run_impl(self,
        ...
        with profile_context(self.name, use_gpu, profiler):
            for i in range(nvprof_end_step + 1):
                if i == nvprof_start_step:
                    core.nvprof_start()
                    core.nvprof_enable_record_event()
                    core.nvprof_nvtx_push(str(i))
                if i == nvprof_end_step:
                    paddle.fluid._cuda_synchronize(paddle.fluid.CUDAPlace(0))
                    core.nvprof_nvtx_pop()
                    core.nvprof_stop()
                    sys.exit()
                if i > nvprof_start_step and i < nvprof_end_step:
                    core.nvprof_nvtx_pop()
                    core.nvprof_nvtx_push(str(i))
                begin = time.time()
    ...
    def run(self, config, args, feeder_adapter=None):
        ...
        outputs, stats = self.run_impl(
            ...
            config=config,
            repeat=args.repeat,
            nvprof_start_step=args.nvprof_start_step,
            nvprof_end_step=args.nvprof_end_step,
