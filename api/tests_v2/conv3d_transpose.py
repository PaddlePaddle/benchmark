...
class Conv3dTransposeConfig(APIConfig):
    ...
    def init_from_json(self, filename, config_id=0, unknown_dim=16):
        ...
        # The argument padding of tf's conv3d_transpose must be a string and
        # the value is "SAME" or "VALID".
        if not isinstance(
                self.padding,
                str) and self.padding != [0, 0, 0
                                          ] and self.padding != [1, 1, 1]:
            self.run_tf = False

        if self.data_format == "NCDHW":
            self.feed_spec[0]["permute"] = [0, 2, 3, 4, 1]
