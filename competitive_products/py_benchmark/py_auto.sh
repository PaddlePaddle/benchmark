...
cp ${PYTORCH_BENCHMARK_ROOT}/scripts/run_scripts/run_detectron.sh ${curl_model_path}/pytorch_detection/
...
echo "git success"
cp ${PYTORCH_BENCHMARK_ROOT}/scripts/run_scripts/run_pix2pix.sh ${curl_model_path}/pytorch-CycleGAN-and-pix2pix
...
echo "----------------pix2pix"
    echo "----1----}"
CUDA_VISIBLE_DEVICES=0 bash run_pix2pix.sh train ${LOG_DIR} > ${RES_DIR}/pix2pix_1.res 2>&1
#sleep 60
#    echo "----8----}" # not run
#CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_pix2pix.sh train ${LOG_DIR} > ${RES_DIR}/pix2pix_8.res 2>&1
...
cp ${PYTORCH_BENCHMARK_ROOT}/scripts/run_scripts/run_stargan.sh ${curl_model_path}/stargan 
...
cp ${PYTORCH_BENCHMARK_ROOT}/scripts/run_scripts/run_image_resnet.sh ${curl_model_path}/pytorch_image_class
cp ${PYTORCH_BENCHMARK_ROOT}/scripts/run_scripts/run_image_senet.sh ${curl_model_path}/pytorch_image_class
...
for model_name in ${model_list[@]}; do
    echo "----------------${model_name}"
    echo "------1-----------}"
    CUDA_VISIBLE_DEVICES=0 bash run_image_resnet.sh speed 32 ${model_name} sp ${LOG_DIR} > ${RES_DIR}/image_${model_name}_1.res 2>&1
    sleep 60
    #echo "------8-----------"  # not run
    #CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_image_resnet.sh speed 32 ${model_name} sp ${LOG_DIR} > ${RES_DIR}/image_${model_name}_8.res 2>&1
done
#------------
echo "----------------se_resnet50"
    echo "----1----}"
CUDA_VISIBLE_DEVICES=0 bash run_image_senet.sh speed 32 se_resnext_50 sp ${LOG_DIR} > ${RES_DIR}/image_se_resnet50_1.res 2>&1
sleep 60
#    echo "----8----}"  #not run 
#CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash run_image_senet.sh speed 32 se_resnext_50 sp  ${LOG_DIR} > ${RES_DIR}/image_se_resnet50_8.res 2>&1 
