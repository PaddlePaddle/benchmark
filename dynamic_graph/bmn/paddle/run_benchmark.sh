...
function _set_params(){
    index=$1
    base_batch_size=8
    model_name="BMN"_bs${base_batch_size}
...
function _train(){
    train_cmd="-c configs/localization/bmn.yaml
               -o epochs=${max_epoch}
    ...
    if [ ${run_mode} = "sp" ]; then
        ...
        rm -rf ./mylog
        train_cmd="python -B -m paddle.distributed.launch --gpus="0,1,2,3,4,5,6,7"  --log_dir ./mylog main.py "${multi_train_cmd}
