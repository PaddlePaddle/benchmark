...
if [[ $# -lt 4 ]]; then
    echo "Usage: "
    echo "  CUDA_VISIBLE_DEVICES=0 bash run_benchmark.sh speed|mem|maxbs 32 model_name sp|mp 1|0(profilerswitch) /ssd1/ljh/logs profiler_path"
    exit
fi
...
function _set_params(){
    ...
    is_profiler=${5}
    run_log_root=${6:-$(pwd)}        # 训练保存的日志目录（必填）
    profiler_dir=${7:-$(pwd)}
...
#    sleep 300
    total_sleep=0
    while [ $total_sleep -le 300 ] && [ `ps -ax | awk '{print $1}' | grep -e "^${train_pid}$"` ] #whether the pid of train.py exists
    do
      sleep 5
