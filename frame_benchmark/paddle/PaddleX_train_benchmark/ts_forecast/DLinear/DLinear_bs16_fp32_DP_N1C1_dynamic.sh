...
train_cmd="python main.py -c paddlex/configs/ts_forecast/DLinear.yaml -o Global.device=gpu:${CUDA_VISIBLE_DEVICES} -o Global.mode=train -o Global.dataset_dir=dataset/ts_forecast_train_benchmark -o Global.output=output/${job_name} -o Train.batch_size=16 -o Train.patience=40 -o Train.epochs_iters=40"
