...
train_cmd="python main.py -c paddlex/configs/text_detection/PP-OCRv4_server_det.yaml -o Global.device=gpu:${CUDA_VISIBLE_DEVICES} -o Global.mode=train -o Global.dataset_dir=dataset/ocr_det_dataset_train_benchmark -o Global.output=output/${job_name} -o Train.batch_size=4 -o Train.log_interval=1 -o Train.epochs_iters=1"
