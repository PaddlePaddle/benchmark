...
train_cmd="python main.py -c paddlex/configs/image_classification/PP-LCNet_x1_0.yaml -o Global.device=gpu:${CUDA_VISIBLE_DEVICES} -o Global.mode=train -o Global.dataset_dir=dataset/ILSVRC2012_train_benchmark -o Global.output=output/${job_name} -o Train.batch_size=512 -o Train.log_interval=1 -o Train.num_classes=1000 -o Train.epochs_iters=1"
