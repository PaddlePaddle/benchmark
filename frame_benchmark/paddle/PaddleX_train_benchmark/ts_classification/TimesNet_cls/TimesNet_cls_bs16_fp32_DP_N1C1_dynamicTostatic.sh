...
export job_name=PaddleX_ts_classification_TimesNet_cls_bs16_fp32_DP_N1C1_d2sT
...
train_cmd="python main.py -c paddlex/configs/ts_classification/TimesNet_cls.yaml -o Global.mode=train -o Global.dataset_dir=dataset/ts_classification_train_benchmark -o Train.epochs_iters=5 -o Benchmark.print_mem_info=True -o Benchmark.disable_deamon=True -o Benchmark.env.FLAGS_eager_delete_tensor_gb=0.0 -o Benchmark.env.FLAGS_fraction_of_gpu_memory_to_use=0.98 -o Benchmark.env.FLAGS_conv_workspace_size_limit=4096 -o Global.model=TimesNet_cls -o Train.batch_size=16 -o Global.device=gpu:${CUDA_VISIBLE_DEVICES} -o Global.output=output/${job_name} -o Train.dy2st=True -o Train.uniform_output_enabled=False"
