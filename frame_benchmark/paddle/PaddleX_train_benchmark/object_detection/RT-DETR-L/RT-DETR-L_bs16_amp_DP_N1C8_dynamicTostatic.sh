...
export model_name=RT-DETR-L_bs16_amp_DP
export job_name=PaddleX_object_detection_RT-DETR-L_bs16_amp_DP_N1C8_d2sT
...
train_cmd="python main.py -c paddlex/configs/modules/object_detection/RT-DETR-L.yaml -o Global.mode=train -o Global.dataset_dir=dataset/coco_train_benchmark -o Train.epochs_iters=1 -o Train.log_interval=1 -o Train.num_classes=80 -o Benchmark.shuffle=False -o Benchmark.shared_memory=True -o Benchmark.print_mem_info=True -o Benchmark.do_eval=False -o Benchmark.disable_deamon=True -o Global.model=RT-DETR-L -o Train.batch_size=16 -o Benchmark.num_workers=16 -o Benchmark.amp=O1 -o Global.device=gpu:${CUDA_VISIBLE_DEVICES} -o Global.output=output/${job_name} -o Train.dy2st=True -o Train.uniform_output_enabled=False"
