...
train_cmd="python main.py -c paddlex/configs/object_detection/PP-YOLOE_plus-L.yaml -o Global.device=gpu:${CUDA_VISIBLE_DEVICES} -o Global.mode=train -o Global.dataset_dir=dataset/coco_train_benchmark -o Global.output=output/${job_name} -o Train.batch_size=16 -o Train.log_interval=1 -o Train.num_classes=80 -o Train.epochs_iters=1"
