...
function _train(){
    ...
    train_options="ILSVRC2012_w --model twins_svt_large --epochs ${max_epochs} --drop 0.0 --drop-path 0.5 --opt adamw --weight-decay 0.05 --lr 1e-3 --min-lr 2e-5 --warmup-epochs 5 --warmup-lr 2e-6 --sched cosine --hflip 0.5 --aa rand-m9-mstd0.5-inc1 --reprob 0.25 --mixup 0.8 --mixup-prob 1.0 --cutmix 1.0 --batch-size ${batch_size} --workers ${num_workers} --log-interval 1"
