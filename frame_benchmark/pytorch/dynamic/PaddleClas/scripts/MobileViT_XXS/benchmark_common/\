...
function _train(){
    ...
    train_options="ILSVRC2012_w --model mobilevit_xs --img-size 256 --epochs ${max_epochs} --opt adamw --weight-decay 0.01 --lr 2e-3 --min-lr 2e-4 --warmup-epochs 5 --warmup-lr 2e-4 --sched cosine --hflip 0.5 --batch-size ${batch_size} --workers ${num_workers} --log-interval 1"
