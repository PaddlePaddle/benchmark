...
function _analysis_log(){
    python analysis_log.py -l ${log_file} -m ${model_item} -b ${batch_size} -n ${device_num} -s ${speed_log_file} -f ${fp_item} --skip_steps ${skip_steps}
}
...
function _train(){
    ...
    train_options="ILSVRC2012_w --model lcnet_100 --epochs ${max_epochs} --weight-decay 3e-5 --lr 0.8 --warmup-epochs 5 --sched cosine --batch-size ${batch_size} --workers ${num_workers} --log-interval 1"
