...
function _train(){
    ...
    train_options="ILSVRC2012_w --model convnext_base --epochs ${max_epochs} --img-size 384 --drop-path 0.1 --opt adamw --weight-decay 0.05 --lr 4e-3 --min-lr 1e-6 --warmup-epochs 20 --warmup-lr 0 --sched cosine --hflip 0.5 --aa rand-m9-mstd0.5-inc1 --reprob 0.25 --mixup 0.8 --mixup-prob 1.0 --cutmix 1.0 --batch-size ${batch_size} --workers ${num_workers} --log-interval 1"
