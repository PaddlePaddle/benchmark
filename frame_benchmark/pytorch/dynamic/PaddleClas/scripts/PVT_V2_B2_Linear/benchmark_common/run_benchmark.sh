...
function _train(){
    ...
    train_config="--config configs/pvt_v2/pvt_v2_b2_li.py"
    train_options="--data-path=../data/ILSVRC2012_val/ \
                   --epochs=${max_epochs} \
    ...
    SingleP) train_cmd="python -u main.py ${train_config} ${train_options}" ;;
    MultiP) train_cmd="python -m torch.distributed.launch --nproc_per_node=8 --master_port=29500 --use_env main.py ${train_config} ${train_options}" ;;
