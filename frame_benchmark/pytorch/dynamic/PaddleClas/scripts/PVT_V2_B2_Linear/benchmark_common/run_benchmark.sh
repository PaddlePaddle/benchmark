...
function _train(){
    ...
    case ${device_num} in
    N1C1) train_cmd="python -u main.py ${train_config} ${train_options}" ;;
    N1C8) train_cmd="python -m torch.distributed.launch --nproc_per_node=8 --master_port=29500 --use_env main.py ${train_config} ${train_options}" ;;
