...
function _set_params(){
    model_item=${1:-"TSM"}   # (必选) 模型 item |fastscnn|segformer_b0| ocrnet_hrnetw48
    base_batch_size=${2:-"30"}       # (必选) 每张卡上的batch_size
    ...
    model_repo="temporal-shift-module"          # (必选) 模型套件的名字
...
function _train(){
    ...
    train_options="kinetics RGB \
                   ...
                   --gd 20 --lr 0.02 --wd 1e-4 --lr_steps 1  \
                   --dropout 0.5 --consensus_type=avg --eval-freq=1 \
                   --shift --shift_div=8 --shift_place=blockres --npb -p=1 \
                   --epochs ${max_epoch} \
