...
function _set_params(){
    model_item=${1:-"maskformer"}   # (必选) 模型 item |fastscnn|segformer_b0| ocrnet_hrnetw48
    ...
    model_repo="MaskFormer"          # (必选) 模型套件的名字
    ...
    num_workers=${7:-"3"}             # (可选)
    train_config=${8:-"detectron_configs/${model_item}.yaml"} # (可选) 模型的训练文件
