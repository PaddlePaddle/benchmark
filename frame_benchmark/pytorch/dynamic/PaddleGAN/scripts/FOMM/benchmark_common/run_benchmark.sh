...
function _train(){
    ...
    train_config="fomm_benchmark_configs/${model_name}.yaml"
    train_options="--no-validate "

    case ${run_process_type} in
    SingleP) train_cmd="python run.py --config  ${train_config}" ;;
    MultiP)
        train_cmd="python run.py --config  ${train_config} --device_ids 0,1,2,3,4,5,6,7" ;;
