...
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
    ...
    if [[ $device_gpu -gt 1 ]]; then
        train="python -m parallel_wavegan.distributed.launch  --nproc_per_node ${n_gpus} --nnodes=$device_gpu --node_rank=$PADDLE_TRAINER_ID --master_addr=$POD_0_IP --master_port=$PORT -c parallel-wavegan-train"
    else
        if [ "${n_gpus}" -gt 1 ]; then
