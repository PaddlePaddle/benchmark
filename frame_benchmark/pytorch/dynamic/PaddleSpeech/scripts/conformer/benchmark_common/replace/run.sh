...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
  ...
  local/download_and_untar.sh ${data} ${data_tiny_url} data_aishell_tiny
  local/download_and_untar.sh ${data} ${data_url} resource_aishell
fi

if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  # Data preparation
  local/aishell_data_prep.sh ${data}/data_aishell_tiny/wav \
    ${data}/data_aishell_tiny/transcript
fi
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  ...
  tools/filter_scp.pl data/train/text \
    $data/data_aishell_tiny/transcript/aishell_transcript_v0.8.txt > $lm/text
