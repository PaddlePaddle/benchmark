...
new_filename="${TEST_DIR}/scripts/benchmark_train.txt"
    ...
    for precision in ${fp_items_list[*]}; do
        for device_num in ${device_num_list[*]}; do
            ...
            if [ ${#gpu_id} -le 1 ];then
                ...
                cmd="bash ${TEST_DIR}/scripts/test_train_inference_python.sh ${FILENAME} benchmark_train > ${log_path}/${log_name} 2>&1 "
                echo $cmd
                eval $cmd
                ...
                cmd="bash scripts/test_train_inference_python.sh ${FILENAME} benchmark_train > ${log_path}/${log_name} 2>&1 "
                echo $cmd
                job_bt=`date '+%Y%m%d%H%M%S'`
                eval $cmd
                job_et=`date '+%Y%m%d%H%M%S'`
