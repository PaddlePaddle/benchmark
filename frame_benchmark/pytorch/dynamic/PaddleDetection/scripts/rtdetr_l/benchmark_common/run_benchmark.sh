...
function _set_params(){
    ...
    model_repo="ultralytics"          # (必选) 模型套件的名字
...
function _train(){
    ...
    train_config="rtdetr-l.yaml"

    case "${run_process_type}" in
    SingleP) train_cmd="yolo detect train data=custom_configs/dataset/coco_benchmark.yaml model=${train_config} val=False epochs=${max_epochs} batch=${batch_size} workers=${num_workers} multi_scale=true optimizer='AdamW' weight_decay=0.0001 ${set_amp}" ;;
    MultiP)
      if [ "${device_num:3}" = '8' ]; then
          batch_size=`expr ${batch_size} \* 8`
          train_cmd="yolo detect train data=custom_configs/dataset/coco_benchmark.yaml model=${train_config} val=False epochs=${max_epochs} batch=${batch_size} workers=${num_workers} multi_scale=true optimizer='AdamW' weight_decay=0.0001 ${set_amp} device=0,1,2,3,4,5,6,7"
      else
