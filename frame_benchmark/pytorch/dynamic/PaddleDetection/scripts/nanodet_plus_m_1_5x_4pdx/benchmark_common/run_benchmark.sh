...
function _analysis_log(){
    python analysis_log.py "${model_item}" "${log_file}" "${speed_log_file}" "${device_num}" "${base_batch_size}" "${fp_item}" '\[NanoDet\]\[(\d\d)-(\d\d) (\d\d):(\d\d):(\d\d)\]INFO:Train\|Epoch'
}
