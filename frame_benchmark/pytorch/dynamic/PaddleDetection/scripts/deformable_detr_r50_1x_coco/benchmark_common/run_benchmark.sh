...
function _set_params(){
    ...
    model_repo="mmdetection"          # (必选) 模型套件的名字
...
function _train(){
    ...
    train_config="configs/deformable_detr/deformable_detr_r50_16x2_50e_coco.py"
    ...
    SingleP) train_cmd="python tools/train.py ${train_config} ${train_options} optimizer.lr=0.0002 " ;;
    MultiP)
      if [ ${device_num:3} = '32' ];then
          train_cmd="python -m torch.distributed.launch --nnodes=${node_num} --node_rank=${node_rank} --master_addr=${master_addr} --master_port=${master_port} --nproc_per_node=8 ./tools/train.py ${train_config} --launcher pytorch ${train_options} optimizer.lr=0.0002"
      elif [ ${device_num:3} = '8' ];then
          train_cmd="python -m torch.distributed.launch --nproc_per_node=8 --master_port=29500 ./tools/train.py ${train_config} --launcher pytorch ${train_options} optimizer.lr=0.0002"
      fi  ;;
