...
function _set_params(){
    model_item=${1:-"pointpillars"}   # (必选) 模型 item |fastscnn|segformer_b0| ocrnet_hrnetw48
    ...
    device_num=${5:-"N1C4"}         # (必选) 使用的卡数量，N1C1|N1C8|N4C8 （4机32卡）
    profiling=${PROFILING:-"false"}      # (必选) Profiling  开关，默认关闭，通过全局变量传递
    model_repo="OpenPCDet"          # (必选) 模型套件的名字
...
function _analysis_log(){
    python analysis_log.py ${model_name} ${base_batch_size} ${log_file} ${speed_log_file} ${device_num}
}
...
function _train(){
    ...
    gpus=${device_num:3:1}
    train_options="--cfg_file=${train_config}  
                   --epochs=${gpus}
                   --workers=${num_workers}
                   --logger_iter_interval=10
                   --num_epochs_to_eval=10000
                   --ckpt_save_interval=10000
                   ${use_amp}
                   --set OPTIMIZATION.BATCH_SIZE_PER_GPU ${batch_size}"

    if [ ${device_num} = "N1C1" ]; then
        train_cmd="python train.py ${train_options}" 
    else
        train_cmd="bash scripts/dist_train.sh ${gpus} ${train_options}"
    fi
