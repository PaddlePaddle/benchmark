...
function _train(){
    ...
    if [ ${device_num:3} = '32' ];then
        train_cmd="accelerate launch --config_file n1c8.yaml --machine_rank $node_rank --main_process_ip $master_addr --main_process_port $master_port train_text_to_image.py ${train_cmd}"
    else
