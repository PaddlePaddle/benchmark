...
def main():
    ...
    if hasattr(accelerator.state, "dynamo_plugin"):
        print("dynamo_backend", accelerator.state.dynamo_plugin)
    else:
    ...
    # log to file!
    logfilepath = os.path.join(logging_dir, f"workerlog.{accelerator.local_process_index}")
    ...
    tokenizer = BertTokenizer.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="tokenizer", revision=args.revision, model_max_length=77
    )
    text_encoder = LDMBertModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="bert", revision=args.revision
    )
    vae = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path, subfolder="vqvae", revision=args.revision)
    ...
    progress_bar = tqdm(range(global_step, args.max_train_steps), disable=progress_bar_disable)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.max_grad_norm > 0:
                    if accelerator.sync_gradients:
                        accelerator.clip_grad_norm_(params_to_train, args.max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        logger.info(f"Saved state to {save_path}", **logger_kwargs)
        ...
        logger.info("train epoch: %d, epoch_cost: %.5f s" % (epoch, train_epoch_cost), **logger_kwargs)
