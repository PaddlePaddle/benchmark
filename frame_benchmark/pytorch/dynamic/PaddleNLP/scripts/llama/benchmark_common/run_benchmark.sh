...
function _set_params(){
    model_item=${1:-"llama"}        # (必选) 模型 item |fastscnn|segformer_b0| ocrnet_hrnetw48
...
function _train(){
    ...
    SingleP) train_cmd="python train_sft.py ${train_cmd}" ;;
    MultiP)
    if [ ${device_num:3} = '32' ];then 
        train_cmd="torchrun --nnodes ${node_num} --nproc_per_node ${num_workers} --node_rank ${node_rank} --master_addr ${master_addr} --master_port ${master_port} train_sft.py ${train_cmd}"
    else
        train_cmd="torchrun --nnodes 1 --nproc_per_node 8 train_sft.py ${train_cmd}"
    fi;;
