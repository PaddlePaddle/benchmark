...
class ExpManagerConfig:
    ...
    # logs preformance of train/val/test steps
    log_step_performance: Optional[bool] = False
...
class PerformanceCallback(Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        self._on_batch_start("train_step_performance")

    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        self._on_batch_end("train_step_performance", pl_module)
    
        elapsed_time = self.timer["train_step_performance"]
...
def exp_manager(trainer: 'pytorch_lightning.Trainer', cfg: Optional[Union[DictConfig, Dict]] = None) -> Optional[Path]:
    ...
    if cfg.log_step_performance:
        performance_callback = PerformanceCallback(timer_kwargs={})
