    ...
    def fit(self):
        ...
        for _ in epoch_iter:
            ...
            for _, global_batch in zip(loop_iter, global_pbar):
                ...
                train_step_time_cur = torch.tensor([train_step_time]).cuda()
                torch.distributed.all_reduce(train_step_time_cur, group=parallel_state.get_data_parallel_group(), op=ReduceOp.MAX)

                # print(global_batch)
                # sum local tokens
                total_tokens = global_batch["chosen"].numel() + global_batch["rejected"].numel()
                ...
                total_tokens_per_second = float(total_tokens) / float(train_step_time_cur)
                total_tokens_per_second_per_device = int(total_tokens_per_second / float(torch.distributed.get_world_size()))
                
                # sum local tokens
                effective_tokens = sum(global_batch["chosen_length"]) + sum(global_batch["rejected_length"])
                # sum tokens cross all data_parallel_group
                ...
                effective_tokens_per_second = float(effective_tokens) / float(train_step_time_cur)
                effective_tokens_per_second_per_device = int(effective_tokens_per_second / float(torch.distributed.get_world_size()))
                
                if parallel_state.get_data_parallel_rank() == 0:
                    print("effective_tokens", float(effective_tokens))
                    print("total_tokens", float(total_tokens))
                    print("effective_tokens_per_second_per_device", effective_tokens_per_second_per_device)
                    print("total_tokens_per_second_per_device", total_tokens_per_second_per_device)

                metrics["effective_tokens_per_second_per_device"] = effective_tokens_per_second_per_device
                metrics["total_tokens_per_second_per_device"] = total_tokens_per_second_per_device
