...
def lm_model(hidden_size, vocab_size, batch_size, num_layers=2, num_steps=20, init_scale=0.1, dropout=None):
    ...
    def padding_rnn( input_embedding, len=3, init_hidden=None, init_cell=None ):
        ...
        for i in range( num_layers ):
            ...
            bias_1 = layers.create_parameter([hidden_size*4], dtype="float32", name="fc_bias1_" + str(i),
                                         default_initializer=fluid.initializer.Constant(0.0))
            bias_arr.append( bias_1 )
            
            pre_hidden = layers.slice( init_hidden, axes=[0], starts=[i], ends=[i+1])
            pre_cell = layers.slice( init_cell, axes=[0], starts=[i], ends=[i+1])
        ...
        with rnn.step():
            ...
            for k in range( num_layers ):
                ...
                j = layers.slice( gate_input, axes=[1], starts=[hidden_size], ends=[hidden_size * 2])
                f = layers.slice( gate_input, axes=[1], starts=[hidden_size * 2], ends=[hidden_size * 3 ])
                o = layers.slice( gate_input, axes=[1], starts=[hidden_size * 3], ends=[hidden_size * 4 ])
                ...
                if dropout != None and dropout > 0.0:
                   input = layers.dropout( input, dropout_prob=dropout)
        ...
        for i in range( num_layers):
            ...
            last_h = layers.slice( m, axes=[0], starts=[num_steps-1], ends=[num_steps] )
            last_hidden_array.append( last_h )
            last_c = layers.slice( c, axes=[0], starts=[num_steps-1], ends=[num_steps] )
    ...
    def encoder_static(input_embedding, len=3, init_hidden=None, init_cell=None ):
        ...
        #real_res = layers.transpose( x=real_res, perm=[1, 0, 2])
        real_res = layers.reshape( real_res, shape=[-1, num_layers, hidden_size], inplace=True)
        last_hidden = layers.concat( hidden_array, 1 )
        last_hidden = layers.reshape( last_hidden, shape=[-1, num_layers, hidden_size] )
        last_hidden = layers.transpose( x = last_hidden, perm = [1, 0, 2])
        last_cell = layers.concat( cell_array, 1)
    ...
    #layers.Print( x, message="xx", summarize=20)
    ...
    #layers.Print( init_hidden, message="init hidden", summarize=20)
    input_size = hidden_size
    x_emb = layers.embedding(
        input=x,
        size=[vocab_size, input_size],
        dtype='float32',
        is_sparse=False,
        param_attr=fluid.ParamAttr(
            name='embedding_para', initializer=fluid.initializer.UniformInitializer(low=-init_scale, high=init_scale) ) )
    
    x_emb = layers.reshape( x_emb, shape=[ -1, num_steps, input_size] )
    #layers.Print( x_emb, message="x_emb", summarize=20)
    if dropout != None and dropout > 0.0:
        x_emb = layers.dropout( x_emb, dropout_prob= dropout, dropout_implementation='upscale_in_train')

    #rnn_out, last_hidden, last_cell = cudnn_lstm( x_emb, len=num_steps,  init_hidden=init_hidden, init_cell=init_cell )  
    ...
    #6770400
    #36024000
    #cudnn_rnn_weight = layers.create_parameter([ 643200 ], dtype="float32", name="cudnn_rnn_weight", \
    #        default_initializer=fluid.initializer.UniformInitializer(low=-init_scale, high=init_scale))
    #print( "!!!", dropout )
    rnn_out, last_hidden, last_cell = layers.cudnn_lstm( x_emb, init_hidden, init_cell,  batch_size,  100, dropout, input_size, hidden_size, num_layers, \
            is_bidirec=False, \
            default_initializer=fluid.initializer.UniformInitializer(low=-init_scale, high=init_scale) )
    #rnn_out, last_hidden, last_cell = padding_rnn( x_emb, len=num_steps,  init_hidden=init_hidden, init_cell=init_cell )  

    #layers.Print( last_hidden, message="last hidden", summarize=20)

    rnn_out = layers.transpose( rnn_out, perm=[1, 0, 2])

    if dropout != None and dropout > 0.0:
        rnn_out = layers.dropout( rnn_out, dropout_prob= dropout, dropout_implementation='upscale_in_train')
    
    rnn_out = layers.reshape( rnn_out, shape=[-1, hidden_size], inplace=True)
    ...
    #loss += layers.reduce_sum( last_hidden) * 0.0 + layers.reduce_sum( last_cell ) * 0.0
    #loss += layers.reduce_sum( last_cell ) * 0.0
