...
class CycleGAN():
    ...
    def train(self):
        ...
        config.gpu_options.allow_growth = True if TEST_GPU_MEM else False
        ...
        with tf.Session(config=config) as sess:
            ...
            writer = FakeWriter() if USE_FAKE_WRITER else tf.summary.FileWriter("./output/2")

            if not os.path.exists(check_dir):
                os.makedirs(check_dir)

            # Training Loop
            for epoch in range(sess.run(self.global_step),40):                
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(0,100):
                ...
                end_t = time.time()
                print('Test time: {}'.format(end_t - start_t))
